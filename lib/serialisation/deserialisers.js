import { getAttributeMetadata, getClassForJsonapiType, getConstructorForJsonapiType, getLinkMetadata, getMetaMetadata, getRelationshipMetadata, } from '../decorators';
import { jsonapiIdentifier, unresolvedIdentifier, } from '../jsonapi';
 ...
 */
export function byTypeAndId(obj) {
 ...
 */
export function fromJsonApiTopLevel(topLevel, resourceObjects = []) {
    ...
    let primaryResourceObjects = undefined;
    ...
    const includedResourceObjects = (resourceObjects || []).concat(included || []);
    const allResourceObjects = primaryResourceObjects.concat(includedResourceObjects);
    const resourceObjectsByTypeAndId = keyBy(allResourceObjects, byTypeAndId);
    const deserialisedObjectsByTypeAndId = {};
 ...
 */
export function fromJsonApiResourceObject(jsonapiResource, resourceObjectsByTypeAndId, deserialisedObjects = {}) {
    // deconstruct primary data and remap into an instance of the chosen type
    const { id, type, attributes = {}, links = {}, meta = {}, relationships = {}, } = jsonapiResource;
    ...
    const extractResourceObject = (linkage, whenNoIncludeRetainIdentifier) => extractResourceObjectOrObjectsFromRelationship(linkage, resourceObjectsByTypeAndId, deserialisedObjects, whenNoIncludeRetainIdentifier);
...
function extractResourceObjectOrObjectsFromRelationship(resourceLinkage, resourceObjectsByTypeAndId, deserialisedObjects, allowUnresolvedIdentifiers) {
    const extractResourceObject = (linkage) => extractResourceObjectFromRelationship(linkage, resourceObjectsByTypeAndId, deserialisedObjects, allowUnresolvedIdentifiers);
